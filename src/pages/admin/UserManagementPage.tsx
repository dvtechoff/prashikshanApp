...
export default function UserManagementPage() {
  ...
  const activateMutation = useMutation({
    mutationFn: () => adminApi.activateUser(Number(userId)),
    onSuccess: () => {
  ...
  const deactivateMutation = useMutation({
    mutationFn: () => adminApi.deactivateUser(Number(userId)),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['admin-user', userId] });
      queryClient.invalidateQueries({ queryKey: ['admin-users'] });
  ...
  const deleteMutation = useMutation({
    mutationFn: () => adminApi.deleteUser(Number(userId)),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['admin-users'] });
  ...
  const displayName = user.profile?.name || user.industry_profile?.company_name || 'No Name';
